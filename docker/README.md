> **Nota.** Instalação canônica (EasyPanel / VPS sem Compose): **[docs/INSTALACAO_COMPLETA.md](../docs/INSTALACAO_COMPLETA.md)**. O `docker-compose.yml` local **não** deve ser usado como modelo de produção: `docker/Dockerfile.db` está duplicado da imagem PHP (não é MariaDB). Banco: serviço MySQL/MariaDB separado + `database/install.sql`.
