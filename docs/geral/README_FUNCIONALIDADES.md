...
- **order_bumps:** produto ofertado no checkout de outro produto (main_product_id, offer_product_id). O preço exibido/cobrado no bump usa `produtos.preco_order_bump` quando `> 0`; caso contrário usa `produtos.preco`. Ver [PRECO_ORDER_BUMP.md](PRECO_ORDER_BUMP.md).
