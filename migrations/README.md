...
- **fix_products_feed_items_duplicates.sql** — Limpa duplicatas de `products_feed_items` (mesmo `usuario_id` + `item_type` + `item_id`) preservando o registro da posição visual atual (`sort_order ASC, id ASC`). Depois cria `UNIQUE unique_user_item (usuario_id, item_type, item_id)` para o UNIQUE com `community_id` NULL não voltar a permitir cópias. Execute manualmente. Instalações novas já recebem o índice em `database/install.sql`.
