-- =============================================================================
-- database/install.sql — Instalador único para BANCO VAZIO
-- Schema atual + seed estrutural mínimo (sem dados de produção).
-- Selecione o banco vazio no cliente SQL e execute este arquivo.
-- NÃO execute em produção. NÃO execute migrations/ em instalação nova.
-- Guia: docs/INSTALACAO_COMPLETA.md
-- =============================================================================
...
-- Selecione o banco VAZIO no cliente SQL ANTES de executar.
-- Sem USE `nome` para o arquivo servir em qualquer nome de banco.

-- Desabilitar FKs para DROP
SET FOREIGN_KEY_CHECKS = 0;
DROP TRIGGER IF EXISTS `after_produto_insert`;
DROP TABLE IF EXISTS `aula_download_term_acceptances`, `aluno_conquistas`, `curso_conquistas`, `curso_gamificacao`, `aula_comentarios`, `alunos_acessos`, `aluno_progresso`, `aluno_ultima_aula`, `aula_arquivos`, `aulas`, `notificacoes`, `vendas`, `gatewaypro_tracking_events`, `gatewaypro_tracking_products`, `modulos`, `cursos`, `order_bumps`, `product_exclusive_offers`, `product_funnels`, `funnel_events`, `checkout_session_events`, `checkout_sessions`, `product_subcategories`, `product_main_categories`, `product_type_categories`, `cupom_produtos`, `cupons`, `produto_ofertas`, `products_feed_items`, `cloned_site_settings`, `cloned_sites`, `saas_assinaturas`, `saas_limites_uso`, `evolution_messages`, `utmfy_integrations`, `webhooks`, `produtos`, `banners`, `licencas_geradas`, `security_events`, `security_logs`, `login_attempts`, `configuracoes`, `configuracoes_sistema`, `communities`, `banner_badges`, `saas_config_admin`, `saas_planos`, `plugins`, `usuarios`, `pwa_config`, `pwa_push_notifications`, `pwa_push_subscriptions`, `payment_decline_logs`;
...
('email_template_delivery_subject', 'Acesso ao seu produto'),
...
(44, NULL, 'checkout_recovery_observe', '0', 'boolean', 'Observação de jornada de checkout (1=on). Default 0. Não dispara mensagens nem altera pagamento.', NOW(), NOW()),
(45, NULL, 'checkout_recovery_enabled', '0', 'boolean', 'Recuperação ativa (WhatsApp/fila). Reservada; manter 0 até autorização. Independente da observação.', NOW(), NOW());
...
CREATE TABLE IF NOT EXISTS `produtos` (
  ...
  `preco_order_bump` decimal(10,2) DEFAULT NULL COMMENT 'Preço quando vendido como Order Bump; NULL = usar produtos.preco',
  ...
  `main_category_id` int(11) DEFAULT NULL COMMENT 'Categoria principal temática (opcional; independente de product_type)',
  `subcategory_id` int(11) DEFAULT NULL COMMENT 'Subcategoria temática (opcional; deve pertencer à categoria principal)',
...
-- --------------------------------------------------------
-- Estrutura: product_main_categories (categoria principal TEMÁTICA por infoprodutor)
-- Independente de product_type. Sem seed: nomes são cadastrados depois pelo usuário autorizado.
-- --------------------------------------------------------
...
-- --------------------------------------------------------
-- Estrutura: product_subcategories (subcategoria vinculada a uma categoria principal)
-- --------------------------------------------------------
...
-- --------------------------------------------------------
-- Estrutura: checkout_sessions (observação de jornada; NÃO é fonte de verdade de pagamento)
-- Sem FK para vendas. Flags checkout_recovery_* nascem em 0.
-- --------------------------------------------------------
...
-- --------------------------------------------------------
-- Estrutura: checkout_session_events (eventos da jornada observada)
-- Sem UNIQUE (session_id, event_name): o mesmo evento pode se repetir.
-- --------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `usuarios` (
  ...
  `session_token` varchar(64) DEFAULT NULL COMMENT 'Token da sessão ativa; novo login invalida sessões anteriores',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
ALTER TABLE `produtos`
  ...
  ADD CONSTRAINT `fk_produtos_main_cat` FOREIGN KEY (`main_category_id`) REFERENCES `product_main_categories` (`id`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_produtos_subcat` FOREIGN KEY (`subcategory_id`) REFERENCES `product_subcategories` (`id`) ON DELETE SET NULL;

--
-- Restrições para tabelas `checkout_sessions`
--
ALTER TABLE `checkout_sessions`
  ADD CONSTRAINT `fk_cs_usuario` FOREIGN KEY (`usuario_id`) REFERENCES `usuarios` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_cs_produto` FOREIGN KEY (`produto_id`) REFERENCES `produtos` (`id`) ON DELETE CASCADE;

--
-- Restrições para tabelas `checkout_session_events`
--
ALTER TABLE `checkout_session_events`
  ADD CONSTRAINT `fk_cse_session` FOREIGN KEY (`session_id`) REFERENCES `checkout_sessions` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_cse_usuario` FOREIGN KEY (`usuario_id`) REFERENCES `usuarios` (`id`) ON DELETE CASCADE;

--
-- Restrições para tabelas `product_main_categories`
--
ALTER TABLE `product_main_categories`
  ADD CONSTRAINT `fk_pmc_usuario` FOREIGN KEY (`usuario_id`) REFERENCES `usuarios` (`id`) ON DELETE CASCADE;

--
-- Restrições para tabelas `product_subcategories`
--
ALTER TABLE `product_subcategories`
  ADD CONSTRAINT `fk_psc_usuario` FOREIGN KEY (`usuario_id`) REFERENCES `usuarios` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_psc_main` FOREIGN KEY (`main_category_id`) REFERENCES `product_main_categories` (`id`) ON DELETE CASCADE;
